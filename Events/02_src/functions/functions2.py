...
def getWFFromFile(evID,path_WF,filetype,fmin, fmax, fs):
    ...
    .
    fmin : str
    type of waveform file (ex. sac gse2 mseed)    
    fmin : int or float
    Minimum frequency for bandpass filter.
    fmax : int or float
    Maximum frequency for bandpass filter.
    ...
    wf = obspy.read(path_WF + str(evID) +  '.' + filetype )
    wf_data = wf[0]
    wf_filter = butter_bandpass_filter(wf_data, fmin,fmax,fs,order=4)
    wf_zeromean = wf_filter - np.mean(wf_filter)
...
def getFeatures_Explore(catalog,pathWF,filetype,fmin,fmax,fs,nfft):
    """
    ...
    columns=['ev_ID','log10RSAM','SC','log10P2P','log10Var','Kurt','DomFreq','Cluster']
    df = pd.DataFrame(columns=columns)
    ...
    for i,evID in enumerate(catalog.event_ID):
        ...
        if i%100==0:
            print(i,'/',len(catalog))
        wf_filter = getWFFromFile(evID,pathWF,filetype,fmin,fmax,fs)

        datetime = pd.to_datetime(catalog.datetime.iloc[i])

        Cluster = catalog.iloc[i].Cluster
        RSAM = np.log10(np.sum(np.abs(wf_filter)))
...
def getFeatures_Explore_preCluster(catalog,pathWF,fmin,fmax,fs,nfft):
    """
    Read waveforms from H5, calculate features prior to SpecUFEx
    Return dataframe with features indexed by datetime
    ...
    columns=['ev_ID','log10RSAM','SC','log10P2P','log10Var','Kurt','DomFreq']
    df = pd.DataFrame(columns=columns)
    ...
    for i,evID in enumerate(catalog.event_ID):
        ...
        if i%100==0:
            print(i,'/',len(catalog))
        wf_filter = getWFFromFile(evID,pathWF,fmin,fmax,fs)
...
def getLocationFeatures(map_catalog,stn,station):
    ...
    columns=['event_ID','datetime','datetime_index','Cluster','Elevation_m','Depth_m','DistXY_m','DistXYZ_m','Azimuth']
    ...
    for i,evID in enumerate(map_catalog.event_ID):
        ...
        Azimuth = azimuthAngle( stnX, stnY, XX, YY)
