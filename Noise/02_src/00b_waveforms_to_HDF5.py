...
##clear group for testing
    ...
    for col in cat_wf.columns:
        ...
        elif col == 'datetime_index':
            catalog_group.create_dataset(name='datetime_index',data=np.array(cat_wf['datetime_index'],dtype='S'))
        else:
    ...
    processing_group.create_dataset(name= "fs", data=sampling_rate)#,dtype='S')
    
    processing_group.create_dataset(name= "sampling_rate_Hz", data=np.ceil(sampling_rate))#,dtype='S')
